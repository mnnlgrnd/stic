...
La logique est similaire à celle d'un [branchement conditionnel simple]({{< ref "cours/04-alternatives.md#branchement-if-simple">}}), à la différence que dans un branchement conditionnel, on passe à la suite du code lorsqu'on finit l'exécution du bloc optionnel. Dans le cas d'une boucle, après une exécution du bloc optionnel, on revient à la condition qui est alors réévaluée pour éventuellement recommencer l'exécution du bloc de code.
